...
ipcMain.on("audio-chunk", async (event, uint8chunk) => {
  const buffer = Buffer.from(uint8chunk);

  const tmpFile = path.join(__dirname, "tmp_chunk.wav");

  try {
    await fs.writeFile(tmpFile, buffer); // Save the chunk
  } catch (writeErr) {
    console.error("FAILED to write tmp file:", writeErr);
    return;
  }
