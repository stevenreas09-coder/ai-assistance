...
async def recognize(websocket):
    ...
    try:
        async for message in websocket:
            ...
            # 2️⃣ Handle PCM audio bytes
            if rec.AcceptWaveform(message):
                result = json.loads(rec.Result())
                text = result.get("text", "").strip()

                if text:
                    print("Final result:", text)  # <-- Add this
                ...
                if partial:
                    print("Partial result:", partial)  # <-- Add this
                    await websocket.send(
                        json.dumps({
                            "partial": partial
                        })
        ...
        # 3️⃣ Flush final result on disconnect
