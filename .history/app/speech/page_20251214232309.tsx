...
export default function Example() {
  ...
  const startStreaming = async (): Promise<void> => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      console.error("WebSocket not open, cannot start streaming");
      return;
    }
    ...
    try {
      ...
      workletNode.port.onmessage = (e) => {
        ...
        if (!listening || !wsCurrent || wsCurrent.readyState !== WebSocket.OPEN)
          return;

        console.log("PCM message bytes:", e.data.byteLength);
        wsCurrent.send(e.data);
      };
  ...
  const toggleListening = (): void => {
    ...
    const newWs = new WebSocket("ws://127.0.0.1:8765");
