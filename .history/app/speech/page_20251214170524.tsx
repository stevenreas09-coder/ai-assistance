...
export default function Example() {
  ...
  const toggleListening = () => {
    // If already connected, close the WebSocket
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      wsRef.current.close();
      return;
    }

    // Only start a new WebSocket if none exists
    if (!wsRef.current) {
      ...
      ws.onclose = () => {
        ...
        setListening(false);
        wsRef.current = null; // clear reference so we can reconnect
      };
      ...
      // Optional: clear transcript when starting fresh
      setTranscript("");
    }
  };
