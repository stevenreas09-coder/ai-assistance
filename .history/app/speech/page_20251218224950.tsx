...
export default function Example() {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setLoading(true);
      setOutput("Reading PDF... please wait.");
      ...
      if (data.text) {
        ...
        // \s+ matches any whitespace (tabs, new lines, spaces)
        // and replaces it with a single space ' '.
        const cleanText = data.text.replace(/\s+/g, " ").trim();

        setResumeText(cleanText); // Save the clean version for the AI

        // Render the clean version in the AI response box for you to see
        setOutput(`📄 RESUME LOADED & CLEANED:\n\n${cleanText}`);
      }
    } catch (error) {
      setOutput("❌ Error reading PDF.");
    } finally {
      setLoading(false);
    }
  };
  ...
  async function handleSend() {
    ...
    const finalMessage = `Instruction: ${instruction}\n\nUser Question: "${transcriptFinal}?"`;
